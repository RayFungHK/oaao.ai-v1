...
| 7 | 核心 app.py 過重（1384 行） | High | P1 | python-lead | 🟡 In progress | W5-S1 phase 1 — admin + health extracted; phase 2 — `/v1/mine/*` + `/v1/research/*` moved; phase 3 — `/v1/live/*` moved; phase 4 — `/v1/runs/{id}/agent_ask|cancel` + `/v1/stream` moved to `routes/runs.py` (registry + `_stream_tokens` into `streaming_state.py`); `/v1/slides/*` moved to `routes/slides.py` (`EndpointPayload` lifted into `routes/_shared_models.py`); phase 5 — `/v1/turn_scores/*` + `/v1/work_queues/status` moved to `routes/turn_scores.py` (`_normalize_score_dims` helper + `TurnScoreRescoreTurn`/`TurnScoreRescoreRequest` models follow); phase 6 — `/v1/asr/transcribe` + `/v1/funasr/{ensure,status}` moved to `routes/asr.py` (`AsrTranscribeRequest`/`FunasrEnsureRequest` models follow); phase 7 — `/v1/skills/discover` moved to `routes/skills.py` (`SkillsDiscoverRequest` model follows; shared `_resolve_api_key*` helpers lifted into `endpoint_keys.py`); app.py 1707→419 LOC; remaining: `/v1/runs/chat` | [python/oaao_orchestrator/app.py](../python/oaao_orchestrator/app.py) |
