...
| 4 | 巨型 PHP controller（單檔 1979 行） | High | P0 | php-lead | ✅ Done | W6-S1 phase 1–2 — `api/*.php` + trait/util; `vault.php` 202 LOC | [backbone/.../vault.php](../backbone/sites/oaaoai/oaaoai/vault/default/controller/vault.php), [VaultControllerSupportTrait.php](../backbone/sites/oaaoai/oaaoai/vault/default/library/VaultControllerSupportTrait.php) |
| 5 | 巨型 Python RAG 模組（1819 行） | High | P0 | python-lead | ✅ Done | W7-S2 phase 2 — `vault_rag/{messages,passages,embed,qdrant}.py`; `vault_graph_rag.py` ~590 LOC | [python/oaao_orchestrator/vault_graph_rag.py](../python/oaao_orchestrator/vault_graph_rag.py) |
| 6 | 巨型執行器模組（1581 行） | High | P0 | python-lead | ✅ Done | W5-S2 phase 1–2 — 15 modules; `run_executor.py` 316 LOC (−82%) | [python/oaao_orchestrator/run_executor.py](../python/oaao_orchestrator/run_executor.py) |
...
| 8 | Queue 仍為 in-process，缺可擴展 backend | High | P1 | python-lead | ✅ Done | W8-S3 — Redis canary Stage 2–4 ops runbook [W8_S3_RedisCanaryRollout.md](W8_S3_RedisCanaryRollout.md); metrics + kill-switch shipped | [python/oaao_orchestrator/queue_backend.py](../python/oaao_orchestrator/queue_backend.py) |
| 9 | vault job 仍依賴 HTTP poll，跨服務耦合高 | High | P1 | python-lead | 🟡 Active | W9 vault job Phase 1 ✅ — dispatch + idle backoff + contract adapter [W9_S1_VaultJobWorkerPhase1.md](W9_S1_VaultJobWorkerPhase1.md); SSE Phase 2 pending | [python/oaao_orchestrator/vault_job_poll.py](../python/oaao_orchestrator/vault_job_poll.py) |
...
| 15 | subprocess 熱點分散，可能造成資源抖動 | Med | P1 | python-lead | ✅ Done | W9-S1 — lane-based subprocess_pool; ASR/ffmpeg + FunASR docker; admin metrics | [python/oaao_orchestrator/subprocess_pool.py](../python/oaao_orchestrator/subprocess_pool.py) |
| 16 | slide template 儲存模組過肥（1149 行） | Med | P2 | php-lead | 🟡 Active | W9-S2 phase 1 — SlideTemplateStoragePaths.php; Html split deferred | [backbone/.../SlideTemplateStorage.php](../backbone/sites/oaaoai/oaaoai/slide-designer/default/library/SlideTemplateStorage.php) |
| 17 | slide project store 過肥（1104 行） | Med | P2 | python-lead | ✅ Done | W9-S2 — store_session.py; store.py 325 LOC | [python/oaao_orchestrator/slide_project/store.py](../python/oaao_orchestrator/slide_project/store.py) |
...
| 19 | CI 缺性能回歸 gate | Med | P2 | qa-lead | ✅ Done | W12-S1 — scripts/perf_regression_gate.sh + CI perf-gate job | [.github/workflows/oaao-ci.yml](../.github/workflows/oaao-ci.yml) |
...
**Burn-down @ 2026-05-27** — Top 20: 18 ✅ Done (#1–8,10–19 except #16 partial) · 2 🟡 Active (#9 phase 2 SSE, #16 Html split, #20) · 0 ⬜ Open · **closed-rate 90%**.
P0 subset (7 items: #1,2,3,4,5,6,14): **7 ✅ — P0 closed-rate 100%**.
