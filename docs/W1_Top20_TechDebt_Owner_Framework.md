...
| 4 | 巨型 PHP controller（單檔 1979 行） | High | P0 | php-lead | 🟡 In progress | W6-S1 phase 1 — `VaultControllerSupportTrait` extracted; `vault.php` ~210 LOC | [backbone/.../vault.php](../backbone/sites/oaaoai/oaaoai/vault/default/controller/vault.php), [VaultControllerSupportTrait.php](../backbone/sites/oaaoai/oaaoai/vault/default/library/VaultControllerSupportTrait.php) |
| 5 | 巨型 Python RAG 模組（1819 行） | High | P0 | python-lead | 🟡 In progress | W7-S2 phase 1 — `vault_rag/{types,embed,qdrant}.py` extracted; main ~1667 LOC | [python/oaao_orchestrator/vault_graph_rag.py](../python/oaao_orchestrator/vault_graph_rag.py) |
| 6 | 巨型執行器模組（1581 行） | High | P0 | python-lead | 🟡 In progress | W5-S2 phase 1 — upstream sampling/timeout extracted to `run_executor_upstream.py`; phase 2 — pipeline-timing helpers extracted to `run_executor_timing.py` (`elapsed_ms_since`, `record_pipeline_phase`, `record_pipeline_task`, `finalize_run_task_timing`); phase 3 — vault-RAG helpers (`vault_rag_ctx_extra`, `apply_vault_rag_agent_result`, `inject_compose_vault_awareness`) extracted to `run_executor_vault_rag.py`; phase 4 — plan/queue helpers (`materials_end_snapshot`, `tool_chain_from_plan`, `plan_pipeline_source`, `reindex_plan`, `insert_tasks_before_llm_stream`, `slide_worker_concurrency`, `pop_parallel_batch`, `slide_page_parallel_batch`, `inject_slide_project_id`, `append_tasks_to_plan`) extracted to `run_executor_plan.py`; phase 5 — VAULT_RAG dispatch branch folded into `handle_vault_rag_task` (run_executor_vault_rag.py); phase 6 — ATTACHMENTS dispatch branch folded into `handle_attachments_task` (run_executor_attachments.py); phase 7 — AGENT dispatch branch folded into `handle_agent_task` (run_executor_agent.py); phase 8 — LLM_STREAM dispatch branch folded into `handle_llm_stream_task` + `LLMStreamState` (run_executor_llm_stream.py); phase 9 — `execute_chat_run` finally block folded into `finalize_run` (run_executor_finalize.py); phase 10 — pre-loop IQS / plan / clarify-shortcut / planner-mode / run-context build folded into `prepare_run_preamble` (run_executor_preamble.py); phase 11 — slide-page parallel-fanout dispatch folded into `handle_slide_page_batch` (run_executor_slide_fanout.py); phase 12 — post-dispatch task housekeeping (status transition + inter-agent handoff + task-end emit + cancellation handshake + report-result replan) folded into `finalize_dispatched_task` (run_executor_post_task.py); phase 13 — run-level error envelope (error-code classify + sanitized detail + Docker hint) folded into `handle_run_error` (run_executor_error.py); phase 14 — chat-attachments entry log + tool_servers registration folded into `run_executor_entry.py`; phase 15 — material-grounding reuse-turn detect + apply folded into `apply_request_material_grounding`; phase 16 — run-task per-type dispatch (VAULT_RAG / ATTACHMENTS / AGENT / LLM_CALL / LLM_STREAM / EMIT / unsupported) folded into `dispatch_run_task` (run_executor_dispatch.py) with `DispatchOutcome` carrying all mutated state; phase 17 — per-task failure envelope folded into `emit_failed_task_end` (run_executor_error.py); run_executor.py 1757→312 LOC (−82%) | [python/oaao_orchestrator/run_executor.py](../python/oaao_orchestrator/run_executor.py) |
| 7 | 核心 app.py 過重（1384 行） | High | P1 | python-lead | ✅ Done | W5-S1 phase 1 — admin + health extracted; phase 2 — `/v1/mine/*` + `/v1/research/*` moved; phase 3 — `/v1/live/*` moved; phase 4 — `/v1/runs/{id}/agent_ask|cancel` + `/v1/stream` moved to `routes/runs.py` (registry + `_stream_tokens` into `streaming_state.py`); `/v1/slides/*` moved to `routes/slides.py` (`EndpointPayload` lifted into `routes/_shared_models.py`); phase 5 — `/v1/turn_scores/*` + `/v1/work_queues/status` moved to `routes/turn_scores.py` (`_normalize_score_dims` helper + `TurnScoreRescoreTurn`/`TurnScoreRescoreRequest` models follow); phase 6 — `/v1/asr/transcribe` + `/v1/funasr/{ensure,status}` moved to `routes/asr.py` (`AsrTranscribeRequest`/`FunasrEnsureRequest` models follow); phase 7 — `/v1/skills/discover` moved to `routes/skills.py` (`SkillsDiscoverRequest` model follows; shared `_resolve_api_key*` helpers lifted into `endpoint_keys.py`); phase 8 — `/v1/runs/chat` moved to `routes/chat.py` (`ChatRunRequest`/`ChatProfilePayload`/`VaultSourceRef` lifted into `chat_models.py`; `_chat_completions_url`/`_sanitize_client_text`/`_resolve_planner_llm`/`_hook_before_llm`/`_report_usage_to_php`/`_shared_secret`/`_php_vault_api_base` lifted into `chat_helpers.py`); app.py 1707→152 LOC (-91%); all 48 routes mount; back-compat re-exports preserved. | [python/oaao_orchestrator/app.py](../python/oaao_orchestrator/app.py) |
...
**Burn-down @ today** — Top 20: 10 ✅ Done (#1,2,3,7,10,11,12,13,14,18) · 3 🟡 In progress (#4,5,6) · 1 🟡 Active (#20) · 2 🟠 Scheduled (#8,9) · 4 ⬜ Open (#15,16,17,19) · **closed-rate 50%**.
P0 subset (7 items: #1,2,3,4,5,6,14): 4 ✅ (#1,2,3,14) · 3 🟡 In progress (#4,5,6) · 0 🟠 Scheduled · 0 ⬜ · **P0 closed-rate 57%**.
