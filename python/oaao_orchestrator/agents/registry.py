...
def default_agent_factories() -> dict[str, AgentFactory]:
    ...
    from oaao_orchestrator.agents.office_generate import OfficeGenerateAgent
    ...
    factories: dict[str, AgentFactory] = {
        ...
        "library_search": lambda: LibrarySearchAgent(),
        "office_generate": lambda: OfficeGenerateAgent(),
    }
    for kind in MM_AGENT_KINDS:
        factories[kind] = mm_media_agent_factory(kind)
    for kind in STUB_AGENT_DEFS:
        if kind in ("slide_designer", "web_search", "library_search", "office_generate") or kind in MM_AGENT_KINDS:
            continue
