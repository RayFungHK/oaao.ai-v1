    ...
    async with httpx.AsyncClient(timeout=60.0) as client:
        ...
        while True:
            try:
                ...
                connect_fail_seq += 1
                if _is_compose_web_boot_wait(host, e) and connect_fail_seq <= _CONNECT_INFO_GRACE:
                    logger.info(
                ...
                elif connect_fail_seq == 1 or connect_fail_seq % _CONNECT_WARN_EVERY == 0:
                    logger.warning(
                        ...
                        claim_url,
                        e,
                        f"repeat x{connect_fail_seq}" if connect_fail_seq > 1 else "first failure",
                    )
                await asyncio.sleep(interval)
            except Exception:
                logger.exception("vault_job_poll: loop error")
                await asyncio.sleep(interval)
            else:
