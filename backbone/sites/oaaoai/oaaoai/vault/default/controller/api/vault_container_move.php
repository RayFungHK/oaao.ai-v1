...
/**
 * POST /vault/api/vault_container_move — reparent a folder within the same vault.
 *
 * JSON body: {@code vault_id}, {@code container_id}, optional {@code parent_container_id} (omit/null = vault root).
 */
return function (): void {
    ...
    if ($parentId !== null) {
        if (! $this->oaao_vault_container_belongs_to_vault($db, $parentId, $vaultId)) {
            http_response_code(400);
            echo json_encode(['success' => false, 'message' => 'Target parent does not belong to this vault.']);

            return;
        }

        $sub = $this->oaao_vault_container_subtree_ids($db, $vaultId, $containerId);
        if (\in_array($parentId, $sub, true)) {
            http_response_code(400);
            echo json_encode(['success' => false, 'message' => 'Cannot move a folder into its own subtree.']);
    ...
    $ts = date('Y-m-d H:i:s');
    $db->update('vault_container', ['parent_container_id', 'updated_at'])
