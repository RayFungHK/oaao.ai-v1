<?php

declare(strict_types=1);

require_once dirname(__DIR__, 2) . '/library/_bootstrap.php';
require_once __DIR__ . '/_internal_auth.php';

use oaaoai\research\ResearchRepository;

/**
 * POST /research/api/source_state_patch — internal: merge incremental/backfill fields into source config_json.
 */
return function (): void {
    header('Content-Type: application/json; charset=UTF-8');

    if (! oaao_research_internal_token_ok()) {
        http_response_code(403);
        echo json_encode(['success' => false, 'message' => 'Forbidden']);

        return;
    }

    $input = json_decode((string) file_get_contents('php://input'), true);
    if (! \is_array($input)) {
        http_response_code(400);
        echo json_encode(['success' => false, 'message' => 'Invalid JSON']);

        return;
    }

    $sourceId = isset($input['source_id']) ? (int) $input['source_id'] : 0;
    $patch = isset($input['patch']) && \is_array($input['patch']) ? $input['patch'] : [];
    if ($sourceId < 1 || $patch === []) {
        http_response_code(400);
        echo json_encode(['success' => false, 'message' => 'source_id and patch required']);

        return;
    }

    $auth = $this->api('auth');
    $db = $auth ? $auth->getDB() : null;
    if (! $db) {
        http_response_code(503);
        echo json_encode(['success' => false, 'message' => 'Database unavailable']);

        return;
    }

    require_once dirname(__DIR__, 4) . '/auth/default/controller/api/_ensure_pg_core_tables.php';
    oaao_auth_ensure_pg_core_tables($db);

    $repo = new ResearchRepository($db);
    $repo->patchSourceConfig($sourceId, $patch);

    echo json_encode(['success' => true], JSON_UNESCAPED_UNICODE);
};
