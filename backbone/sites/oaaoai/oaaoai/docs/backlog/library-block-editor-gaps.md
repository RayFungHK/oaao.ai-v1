...
| L-ED-4 | Upload file → full `POST /v1/library/convert` (docx/pdf, not text stub) | CS-2-S3 ✅ upload API + vault extract |
...
| L-ED-8 | Import text → RazyUI `Dialog` + `AjaxForm` (replace `window.prompt`) | UX ✅ Dialog import |
